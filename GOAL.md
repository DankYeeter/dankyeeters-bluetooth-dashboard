...
- **AK-7 — ZURUECKGESTELLT, Nummer reserviert.** Vorgesehen war: "Die
  Funkstrecke wird gezeigt, nicht erraten" — die Anzeige stuetzt sich auf die
  Firmware-Telemetrie aus dem BQR-Abschnitt (Pakettyp, RSSI,
  Sendeleistungsstufe, Wiederholrate, Nicht-Empfang, AFH-Kanalauslass) statt
  auf Rueckschluesse aus der Sendewarteschlange.
  **Entscheidung des App Designers 2026-09-03: erst nach der
  Quelltext-Klaerung festschreiben.** Grund: T-032 deutet darauf hin, dass
  das Lesen des BQR-Abschnitts die Ereignis-Queue leert. Traefe das zu, wuerde
  eine pollende App die Telemetrie loeschen, die sie anzeigen will, und sie
  jedem anderen Leser wegnehmen — ein Eingriff, kein Instrument, und ein
  Zusammenstoss mit AK-1. Klaerung laeuft als T-033 → `docs/research/R-011.md`.
  **Bis dahin wird an diesem Kanal nichts gebaut.** Die Nummer bleibt
  reserviert, damit sich AK-8..AK-16 nicht verschieben.
...
**2026-09-03, nachtraeglich — Abnahme durch den App Designer.**
Der Entwurf `docs/GOAL-entwurf-part5.md` wurde durchgesprochen und mit zwei
Aenderungen angenommen; diese Datei ist das Ergebnis. Die zehn neuen Kriterien
gehen damit **neun** in Kraft:

6. **AK-7 zurueckgestellt** bis zur Quelltext-Klaerung des BQR-Read-Clear
   (T-033/R-011). Nummer reserviert. Grund siehe beim Kriterium selbst.
7. **AK-14 umformuliert** auf "ohne kuenstliche Stoerung". Grund: kein
   belegter Stresshebel. Damit ist die Testsuite nicht mehr blockiert.
