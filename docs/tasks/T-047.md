...
## T-047f — S3-3 Ledger-Kern (developer)

**Stufe:** normal. **Isolation:** Worktree. **Schritt 0:** im Worktree
`git reset --hard auto/2026-09-23-saeule3`, dann `git log --oneline -1` muss
`b9c8202` oder später zeigen; `local.properties` aus dem Hauptbaum kopieren
(ungetrackt, `sdk.dir`). Commits auf eigenem Branch, der Director führt zusammen.
**Vorlauf:** Security T-047b, Urteil CONCERNS — Vorgaben M1–M7 unten sind
Pflicht. Entscheide D1–D4 im Kopf dieser Datei gelten.
**Berührt Dateien:** neu `devices/SettingsLedger.kt`, `DeviceProfileApplier.kt`,
`DeviceProfileStore.kt`, `GlobalSettingsController.kt` (M1, Lesen mit drei
Ausgängen); Verdrahtung `SystemGraph.kt`, `backup_rules.xml`,
`data_extraction_rules.xml`; Tests. **Parallel:** ja, kein anderer developer
berührt diese Dateien.
**Scope-Grenzen:** kein Rückweg (S3-5), keine UI ausser der Konstante für den
Skip-Grund, kein `LdacTuning`.

### Vorgabe wörtlich: AD-033 — steht oben in Abschnitt T-047b
### Vorgabe wörtlich: AD-038 Zeile, Schnittstelle, Testfälle S3-3
|---|---|---|---|---|
| **S3-1** | P-2 | `A2dpLinkDumpParser.kt`, `LinkLiveModels.kt` (`LiveLinkSource.kt`: Feld durchreichen) | nein | nichts (Anzeige in S3-6) |
| **S3-4** | P-4 | neu `app/.../ui/tuning/EnvironmentConditions.kt` | nein (liest nur) | nichts (Anzeige in S3-6) |
- **S3-3:** Applier bekommt `ledger: SettingsLedger = NoSettingsLedger` als
...
### Vorgabe wörtlich: Security-Vorlauf T-047b, S3-3
...
### Vorgabe wörtlich: UI_SPEC.md S3-3 und AK-T047-1
...
## Hinweis des Directors zu T-047d..f (23.09. 18:35)

Die in T-047d/e/f kopierten Tabellenzeilen aus AD-038 sind um eine Zeile
verschoben (T-047e zeigt die S3-3-Zeile, T-047f die S3-4-Zeile). Massgeblich
waren und sind die Dateilisten und Schnittstellen im Abschnittstext. Ab hier
korrekt.

## Welle 2 — gemeinsame Vorgaben (T-047g, T-047h)

**Schritt 0:** im Worktree `git reset --hard saeule3-int` (Integrationszweig
mit S3-1, S3-2, S3-3 zusammengeführt), dann `git log --oneline -1` muss
`7d1fb92` oder später zeigen; `local.properties` aus dem Hauptbaum kopieren.
Commits auf eigenem Branch, der Director führt zusammen. Kein adb.
Die Auftragsdatei liegt im Hauptbaum:
`C:\Users\Daniel\Desktop\ClaudeCode\dankyeeters-bluetooth-dashboard\docs\tasks\T-047.md`.

## T-047g — S3-4 Zustandsbuch-Leser (developer)

**Stufe:** klein (eine neue Datei mit Logik, Schnittstelle und Testfälle unten).
**Vorlauf:** T-047b M17 (unten), sonst kein Auslöser (liest nur).
**Berührt Dateien:** neu `app/.../ui/tuning/EnvironmentConditions.kt`, Tests.
**Parallel:** ja, T-047h disjunkt. **Scope-Grenzen:** keine Anzeige (S3-6),
kein `WifiManager`, keine neue Permission, `ble_scan_always_enabled` kommt
nicht ins Zustandsbuch (AD-038).

### Vorgabe wörtlich: AD-038 Tabellenkopf und Zeile S3-4, Testfälle S3-4
...
## T-047h — S3-5 Rückweg (developer)

**Stufe:** normal. **Vorlauf:** Security T-047b, CONCERNS — M8–M15 und die
Darf-nicht-Liste unten sind Pflicht; D1–D4 im Kopf gelten; AD-034 steht oben
in T-047b. **Zusätzlich (Frage aus T-047f):** die drei Schreiber ausserhalb des
Appliers — `DeviceProfilesViewModel.kt:310` (Globals), `:478` (Absolute
Volume), `LdacTuning.pin` aus `DeviceProfilesViewModel.kt:502` und
`MonitorViewModel.kt:380` — bekommen in diesem Lauf den Ledger-Eintrag vor
dem Schreiben (AD-038 S3-5, M15). Bestand aus S3-3 (Commit `4b6ad5b`):
`SettingsLedger.entries()` gibt `Result<List<LedgerEntry>>`, das Interface hat
`val lock: Mutex`, `remove` wirft bei unlesbarem Ledger.
**Berührt Dateien:** `LdacTuning.kt`, neu `app/.../ui/tuning/SettingsRestore.kt`,
`DeviceProfilesViewModel.kt`, `DeviceProfilesScreen.kt`, `MonitorViewModel.kt`
(nur der `pin`-Aufruf); Verdrahtung App-Graph, `BluetoothScreen.kt`/
`ActivateRoute` für den Banner (D2, M14); Tests. **Parallel:** ja, T-047g
disjunkt. **Scope-Grenzen:** kein Vergleichsablauf (S3-6), keine
`ComparisonSection`.
**Offene Frage App Designer (nicht blockierend):** zweiter Rückweg-Knopf im
Vergleichsergebnis — betrifft S3-6, nicht diesen Lauf.

### Vorgabe wörtlich: AD-038 Tabellenkopf, Zeile, Schnittstelle, Testfälle S3-5
| Lauf | P | Dateien mit Logik (Verdrahtung) | schreibt Einstellungen → Security-Vorlauf | UI-Spec liefert |
|---|---|---|---|---|
| **S3-5** | P-5 (Rueckweg) | `LdacTuning.kt`, neu `app/.../ui/tuning/SettingsRestore.kt`, `DeviceProfilesViewModel.kt`, `DeviceProfilesScreen.kt` (Instanz in der App-Verdrahtung) | **ja** — schreibt Globals, HD-Audio, Codec zurueck; setzt Autoapply aus | Knopf „zurueck auf vorher" (Bluetooth-Tab), Bestaetigung, Bericht: zurueckgestellt / nicht zurueck mit Grund / Autoapply ausgesetzt fuer {Profil} / Codec-Familie erst beim naechsten Verbinden; Dauerhinweis fuer offene Eintraege mit „erneut versuchen" |
- **S3-5:** `LdacTuning.pin` haelt vor `store`/`apply` `LedgerEntry.Ldac(key,
...
### Vorgabe wörtlich: UI_SPEC.md D2, S3-5, AK-T047-2..5
...
## Welle 3 — gemeinsame Vorgaben (T-047i, T-047j)

**Schritt 0:** im Worktree `git reset --hard saeule3-int`, dann
`git log --oneline -1` muss `6f192a3` oder später zeigen (S3-1..S3-5
zusammengeführt); `local.properties` aus dem Hauptbaum kopieren. Commits auf
eigenem Branch, der Director führt zusammen. Kein adb.

## T-047i — S3-6 Geführter Vergleich (developer)

**Stufe:** normal. **Vorlauf:** T-047b S3-6 (M16–M18, Darf-nicht) unten ist
Pflicht; D1–D4 gelten; AD-036 steht oben in T-047c.
**Bestand aus S3-5 (Commit `86ca3ab`):** `LdacTuning.pin` liest die Live-Stufe
selbst und hält den Ledger-Eintrag vor dem Schreiben; M16 verlangt zusätzlich,
dass `pin` ein Ergebnis zurückgibt (heute `Unit`, still zurück bei `busy`).
**Offene Fragen App Designer (UI_SPEC.md:3512), Director-Vorgabe bis zur
Antwort:** Kategoriename „Workarounds" und zweiter Rückweg-Knopf im Ergebnis —
beide nach Empfehlung der Spec bauen; Änderung später ist ein Textwechsel.
**Risiko AD-038 (Pflichttest):** Tabwechsel darf Arm A nicht verwerfen — Test,
der Arm A über einen Tabwechsel hält; sonst Controller eine Ebene höher
(Activity-Scope), keine Persistenz.
**Berührt Dateien:** neu `ComparisonController.kt`, neu `ComparisonSection.kt`,
`LiveLinkPanel.kt` (Paarungsfakten), `LdacTuning.kt` (nur Rückgabewert `pin`,
M16); Verdrahtung `MonitorViewModel.kt`, `MonitorScreen.kt`; Tests.
**Parallel:** ja, T-047j disjunkt.

### Vorgabe wörtlich: AD-038 Tabellenkopf, Zeile, Schnittstelle, Testfälle S3-6
| Lauf | P | Dateien mit Logik (Verdrahtung) | schreibt Einstellungen → Security-Vorlauf | UI-Spec liefert |
|---|---|---|---|---|
| **S3-6** | P-8, P-9, Anzeige P-2 | neu `ComparisonController.kt`, neu `ComparisonSection.kt`, `LiveLinkPanel.kt` (Paarungsfakten) (`MonitorViewModel.kt`, `MonitorScreen.kt`) | **ja** — pinnt 990 ueber den vorhandenen `LdacTuning.pin`-Pfad | Einstieg und Hinweis (F1: Aussetzer hoerbar); Massnahmenliste (6) und eigene Sektion „Ausweichen" (4) mit Begruendung, warum keine Behebung — Kategoriename muss als „Ausweichen" erkennbar sein; Anleitung je Massnahme; Rueckleseworte (bestaetigt / widerspricht / nicht pruefbar); Armfortschritt „{T} von 15 min beobachtet"; Armende-Gruende (Bestand + `TARGET_REACHED`); vier Urteilssaetze + je `NotComparable` ein Satz; **Messrahmen am Ergebnis**: beide Dauern, Stufe, Codec, Kadenz, Zustandsbuch Anfang/Ende je Arm, `notChecked`, Drift-Satz (F2), „relative Aussage ueber genau diese Bedingungen, keine Aussage ueber Alltagsqualitaet", Nachweisgrenze aus `smallestDetectableBaseline()`; AK-16-Satz; Paarungsfakten-Zeilen; Satz zum Prozesstod; Rueckweg-Knopf. R-A, R-F, R-G gelten. |
- **S3-6:** `ComparisonController(scope)` nach dem Muster `ObservationRunController`:
...
### Vorgabe wörtlich: UI_SPEC.md S3-6, Zustände, Regelprüfung, AK-T047-6..17
...
## T-047j — Nachzug SR-025 und Testlücke M10 (developer)

**Stufe:** klein. Befunde aus T-047h (developer, 23.09. 18:50), wörtlich:
1. „`GlobalSettingsController.clear` bestätigt über `read()`, und das macht aus
   „nicht lesbar" ein „nicht gesetzt". Der Applier meldet ein Zurücksetzen
   deshalb auch bei unlesbarem Nachlesen als Erfolg. Die Korrektur ist eine
   Zeile (`readState`)." — Security SR-025: „`clear()` prüft über `read()`
   (`GlobalSettingsController.kt:72-73`). Ein Lesefehler gilt dort als
   „gelöscht"." Klasse: jede Bestätigung über `read()` nach einem Schreiben in
   `GlobalSettingsController.kt` und `SettingsRestore.kt`; Treffer behebt
   derselbe Lauf.
2. „`DeviceProfileStore` liefert bei Lesefehlern still eine leere Liste. Das
   Nachlesen des Profils bei `priorWish == null` könnte dadurch einen Erfolg
   vortäuschen." — Umfang: nur der Nachlesepfad in `SettingsRestore.kt` darf
   einen Lesefehler nicht als Erfolg werten; das allgemeine Verhalten von
   `DeviceProfileStore` bleibt.
3. „Lücke, kein Beleg — M10: Dass das Aussetzen von Autoapply fehlschlägt,
   lässt sich mit dem echten `DeviceProfileStore` nicht auslösen." — Test mit
   einer Attrappe, deren Speichern fehlschlägt: kein Schreibaufruf, Grund im
   Bericht. M10 wörtlich: „Zuerst wird Autoapply ausgesetzt, und zwar
   gespeichert und abgewartet. Schlägt das Speichern fehl, schreibt der
   Rückweg nichts und meldet den Grund."
Je Punkt ein negativer Test, der ohne Fix rot ist (Mutationsbeleg im Bericht).
**Vorlauf:** T-047b (diese Punkte sind seine Auflagen M9/M10).
**Berührt Dateien:** `GlobalSettingsController.kt`, `SettingsRestore.kt`, Tests
(ggf. eine Test-Attrappe). **Parallel:** ja, T-047i disjunkt.
